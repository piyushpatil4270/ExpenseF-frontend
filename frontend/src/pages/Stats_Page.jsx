...
const Stats_Page = () => {
    ...
    async function groupInArray2(month){
        ...
        for(let i=0;i<month.length;i++){
            const currMonth = month[i].month;
            const monthIndex = moment(currMonth, 'YYYY-MM').month(); 
            const totalAmount = parseInt(month[i].totalAmount);
            temp[monthIndex] += totalAmount;
        }
        setMonthArray(temp)
    }
    const fetchLeaderboard = async () => {
        try {
            ...
            const res = await axios.get("https://www.expensetracker.kesug.com/expense/leaderboard", { headers: { "Authorization": userToken } });
            setdata(res.data);
        } catch (error) {
            console.log(error);
        }
    ...
    const fetchStats = async () => {
        try {
            const userToken = localStorage.getItem("token");
            const res = await axios.get("https://www.expensetracker.kesug.com/expense/expenseStats", { headers: { "Authorization": userToken } });
            console.log("The data of stats is ", res.data);
            await groupInArray(res.data?.categoryStats);
            await groupInArray2(res.data?.monthlyStats)
